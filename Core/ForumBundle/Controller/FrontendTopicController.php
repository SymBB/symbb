...
class FrontendTopicController extends \SymBB\Core\SystemBundle\Controller\AbstractController
{
    ...
    public function showAction($name, $id)
    {
        ...
        $pagination = $paginator->paginate(
            $query, $this->get('request')->query->get('page', 1)/* page number */, $topic->getForum()->getEntriesPerPage()
        );
        ...
        if ($lastPageFirst) {
            ...
            $query = $em->createQuery($dql);
            $pagination = $paginator->paginate(
                $query, $lastPage, $topic->getForum()->getEntriesPerPage()
            );
        }
    ...
    public function deleteAction($topic)
    {
        ...
        $this->get('session')->getFlashBag()->add(
            'success', $this->get('translator')->trans('Your post has been deleted successfully. You will now be redirected ...', array(), 'symbb_frontend')
        );
    ...
    protected function getTopicForm($forum, &$post, &$topic)
    {
        ...
        $form = $this->createForm(new \SymBB\Core\ForumBundle\Form\Type\TopicType($url, $topic, $dispatcher, $this->get('translator'), $this->get('doctrine.orm.symbb_entity_manager'), $this->get('symbb.core.user.manager'), $this->get('symbb.core.group.manager')), $topic);
    ...
    protected function getPostForm($topic)
    {
        ...
        $dispatcher = $this->get('event_dispatcher');
        $form = $this->createForm(new \SymBB\Core\ForumBundle\Form\Type\PostType($url, $post, $dispatcher, $this->get('translator'), $this->get('doctrine.orm.symbb_entity_manager'), $this->get('symbb.core.user.manager'), $this->get('symbb.core.group.manager')), $post);
