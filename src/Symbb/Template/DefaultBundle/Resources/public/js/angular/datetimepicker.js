...
angular.module('ui.bootstrap.datetimepicker', [])
    ...
    .constant('dateTimePickerConfigValidation', function (configuration) {
        ...
        var validOptions = ['startView', 'minView', 'minuteStep', 'dropdownSelector', 'weekStart', 'viewModel'];
        ...
        if (configuration.viewModel !== null && !angular.isString(configuration.viewModel)) {
            throw ("viewModel must be a string");
        }
            ...
            link: function (scope, element, attrs) {
                ...
                var dataFactory = {
                    year: function (unixDate) {
                        ...
                        var activeYear = scope.ngModel ? moment(scope.ngModel * 1000).year() : 0;
                    ...
                    month: function (unixDate) {
                        ...
                        var activeDate = scope.ngModel ? moment(scope.ngModel * 1000).format('YYYY-MMM') : 0;
                    ...
                    day: function (unixDate) {
                        ...
                        var activeDate = scope.ngModel ? moment(scope.ngModel * 1000).format('YYYY-MMM-DD') : '';
                    ...
                    hour: function (unixDate) {
                        ...
                        var activeFormat = scope.ngModel ? moment(scope.ngModel * 1000).format('YYYY-MM-DD H') : '';
                    ...
                    minute: function (unixDate) {
                        ...
                        var activeFormat = scope.ngModel ? moment(scope.ngModel * 1000).format('YYYY-MM-DD H:mm') : '';
                    ...
                    setTime: function (unixDate) {
                        ...
                        scope.ngModel = newDate.getTime() / 1000;
                        if (configuration.viewModel) {
                            try {
                                eval('scope.$parent.' + configuration.viewModel + ' = moment(newDate).format("LLLL")');
                            } catch (myExc) {
                                console.debug(myExc);
                            }
                        }
                ...
                var getUTCTime = function () {
                    var tempDate = (scope.ngModel ? moment(scope.ngModel * 1000).toDate() : new Date());
                ...
                scope.changeView = function (viewName, unixDate, event) {
                    if (event) {
                        ...
                        var temp = configuration.viewModel.split('.');
                        var temp2 = '';
                        for (var i = 0; i < temp.length; i++) {
                            temp2 = temp2 + '.' + temp[i];
                            if (i === (temp.length - 1)) {
                                eval('if(!scope.$parent' + temp2 + ') { scope.$parent' + temp2 + ' = 0; }');
                            } else {
                                eval('if(!scope.$parent' + temp2 + ') { scope.$parent' + temp2 + ' = [] }');
                            }
                        }
                        var tempDate = new Date(unixDate);
                        var newDate = new Date(tempDate.getTime() + (tempDate.getTimezoneOffset() * 60000));
