# SymbbCoreBBCodeBundle

This is the BBCode Bundle.
Read about BBCode in googe :) in short its a own syntax to format text.
With very less posibilities for the user and completly without html so that the user cannot create ugly or not working html stuff.
Its the standard in every Forumsystem. But i think we will add also a html editor so that the admin can say that the user can use bbcode in Forum A but in Forum B he can use HTML.
( E.g for some Tutorial Forum it will be nice to have more options to create a nice looking post )
